...
*Updated again: Phase 54 is now done (see below - a structured,
minidump-inspired crash dump, closing the "tells you why when it
fails" row in 1.3), on top of Phase 53 (a journaled FAT32, closing the
"won't corrupt data" row in 1.3) and the 50-52 update before that -
kernel work only, matching your stated priority; nothing in Part 2
(userland) has changed since this document was last updated, so that
section is unmodified. Every "done" mark below
