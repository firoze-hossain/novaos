...
| `ping`/`nslookup`/`tftp`/`pkg`/the GUI aren't reachable from the ring-3 shell yet. | ✅ **Done (Phase 60), the two genuinely missing pieces written entirely in Rust.** By the time this phase started, `ping`/`pkg`/`gui` (a proof-of-concept demo, not the full compositor — a separate, still-open gap under 2.3) were already wired into the ring-3 shell by earlier phases (33, 32, 32b) — this row's own text hadn't been updated since. Only `nslookup` and `tftp` were actually absent; both are now real, separate ring-3 ELF32 programs (`userland/net-rs/nslookup.rs`, `tftp.rs`), following Phase 59's own "own directory, shared `ffi.rs`, own `build.sh`" shape exactly. Two new syscalls back them: `SYS_DNS_RESOLVE` (wraps the existing `kernel/net/dns.c`) and `SYS_TFTP_FETCH` (wraps the existing `kernel/net/tftp.c`, gated on `can_open_any_file` since it creates/overwrites a local file — `tftp` launches via `SYS_EXEC_TRUSTED` the same way `cp`/`rm` do; `nslookup` needs no special capability, like `ping`). Exposing these two existing, previously kernel-boot-only C functions to a syscall caller surfaced a real, latent bug: both had the exact same "`int 0x80` disables interrupts, so an un-yielding busy-wait spins forever" hazard Phase 58 found and fixed in `arp_resolve()` — fixed here the identical way (`scheduler_yield()` in each wait loop) *before* either syscall shipped, not discovered afterward. **Verified**: the kernel builds clean with both new syscalls, the full boot-test suite shows no regressions (same 9 pre-existing, environment-caused failures as before this phase), `shell.c`'s new dispatch code compiles warning-free, and both new Rust programs pass a zero-warning host-target metadata check — this sandbox's same pre-existing `compiler_builtins` rlib mismatch (Phase 33 onward) means the actual `.ELF` binaries need building on the user's own machine via `sh userland/net-rs/build.sh`, the same gap Phase 59's four coreutils are already in. | Right now the "real" shell (ring-3) is meaningfully less capable than the old ring-0 one — a visible regression to anyone who's used both. | — |
